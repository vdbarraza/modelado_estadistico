...
trabajo es  **predecir la calificación de IMDB para las películas** a partir de otras covariables para cada título.
Consideraremos la pérdida cuadrática como forma de evaluar modelos.
...
# make this example reproducible
set.seed(1)
glmnet2.train<-  train(imdb_score ~ . , data=Features_train[,-c(20,23,24)],method="glmnet",tuneLength=5,family="gaussian",
trControl=trainControl(method="cv",number=3))

glmnet2.train %>% summary()
...
```{r fig.cap=" Relación entre  las predicciones de IMDB_SCORE y el año de estreno para las películas", out.width="50%", fig.align="center",out.width="80%"}
test_1<- ggplot(merged_test, aes(x=pred,y =release_year )) +
  geom_point()+
  theme_bw()+
  labs(x = "Imdb rating", y= "Count", title = "Distribution of Imdb rating by movie")

train_1<- ggplot(Features_test, aes(x=lmer_pred,y =release_year)) +
  geom_point()+
  theme_bw()+
  labs(x = "Imdb rating", y= "Count", title = "Distribution of Imdb rating by movie")
...
```{r fig.cap=" Relación entre  las predicciones de IMDB_SCORE y el runtime para las películas", out.width="50%", fig.align="center", out.width="80%"}

test_1<- ggplot(merged_test, aes(x=pred,y =runtime )) +
  geom_point()+
  theme_bw()+
  labs(x = "Imdb rating", y= "Count", title = "Distribution of Imdb rating by movie")

train_1<- ggplot(Features_test, aes(x=lmer_pred,y =runtime)) +
  geom_point()+
  theme_bw()+
  labs(x = "Imdb rating", y= "Count", title = "Distribution of Imdb rating by movie")
